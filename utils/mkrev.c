...
void initrevfile(FILE *f,int cctimes)
	...
	fprintf(f,"static char _IDstring[1]={0};\n");
	...
	fprintf(f,"#else\n");
	fprintf(f,"static char _IDstring[]=\"\\0$VER: %s \"VNUM\" (%02d.%02d.%d) [c: %d] © by Samuel Devulder.\";\n",
		VNAME,tm->tm_mday,tm->tm_mon+1,tm->tm_year,cctimes);
	fprintf(f,"#endif\n");
	fprintf(f,"char *IDstring=&_IDstring[7];\n\n");
...
static int cmpfunc(char **a,char **b)
	{
	return strcmp(*a,*b);
	}

static void getlinkfile(char *linkfile,int *pac,char ***pav)
	{
	int ac=2,i=2;
	char **av=NULL;
	char line[80];
