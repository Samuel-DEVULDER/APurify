/*
 * dcc.c - dcc part of apurify frontend.
 *
...
/*
 * determine if dcc arg and pattern are equal. Eventually split it when needed
 *
 * if pattern ends with ' ' that means a second argument is optionally
 * requiered but may be concatenated.
 */
PRIVATE int dcc_arg(int ac,char *pattern)
	{
	char *a=AV[ac];
...
void dcc_parse(void)
	...
	for(ac=1;ac<AC;++ac)
		...
		else if(dcc_arg(ac,"-c"))
			{
			if(LinkIdx) goto errlink;
			LinkIdx = ac;
			PhaseTo = PHASE_O;
			}
		else if(dcc_arg(ac,"-d "))
			{
			seen_d = TRUE;
			}
		else if(dcc_arg(ac,"-a"))
			{
			if(LinkIdx)
				{
errlink:			printf("%d: Arg \"%s\" illegal (\"%s\" is used)\n",
	...
	ifn(DestIdx) /* create a dest entry */
		...
		if(PhaseTo == PHASE_E) dest = s_strdup("a.out");
	...
	if(!seen_d && source_level) add_arg("-d1");
	}
